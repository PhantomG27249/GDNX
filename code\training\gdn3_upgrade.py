...
# Resolve imports for both standalone and package usage
import sys, os
...
from gdn3_production.kernels import (
    kron_read_pytorch,
    stable_alpha,
...
class GDN3LinearAttn(nn.Module):
    ...
    def __init__(self, config, layer_idx: int = 0):
        ...
        self.residual_rank = 16                       # Exact write buffer P
    ...
    def _generate_coproduct_channels(
        ...
        # Coproduct q/k: W_a[H,C,a_k,D], W_b[H,C,b_k,D] -> [B,T,H,C,a_k] + [B,T,H,C,b_k]
        q_a = torch.einsum('hcid,btd->bthci', self.W_q_a.float(), x_f)  # [B,T,H,C,a_k]
        q_b = torch.einsum('hcjd,btd->bthcj', self.W_q_b.float(), x_f)  # [B,T,H,C,b_k]
        ...
        k_a = torch.einsum('hcid,btd->bthci', self.W_k_a.float(), x_f)
        k_b = torch.einsum('hcjd,btd->bthcj', self.W_k_b.float(), x_f)
        ...
        v_a = torch.einsum('hcid,btd->bthci', self.W_v_a.float(), x_f)  # [B,T,H,C,a_v]
        v_b = torch.einsum('hcjd,btd->bthcj', self.W_v_b.float(), x_f)  # [B,T,H,C,b_v]
    ...
    def _apply_partial_rope(self, x: torch.Tensor, t_max: int) -> torch.Tensor:
        ...
        for m_idx in range(self.M):
            ...
            # Get RoPE for this lane
            if scale != 1.0:
                inv_freq = 1.0 / (10000.0 ** (
                    torch.arange(0, d_pairs, device=x.device, dtype=torch.float32) /
                    max(1, int(self.K * 0.25) // 2 - 1)
                ))
                angles = (torch.arange(min(T, t_max), device=x.device, dtype=torch.float32) * scale)[
                    :, None] * inv_freq[None, :]
            else:
                angles = torch.zeros(T, d_pairs, device=x.device)
                if d_pairs <= self.cos_pe.shape[1]:
                    angles = (torch.arange(min(T, self.cos_pe.shape[0]), device=x.device,
                                          dtype=torch.float32) * scale)[:, None] * \
                             (1.0 / (10000.0 ** (torch.arange(0, d_pairs, device=x.device,
                                                              dtype=torch.float32) /
                                                  max(1, d_pairs - 1))))[None, :]
    ...
    def _gdn3_recurrent_state(
        ...
        comp_err = 0.0
        for t in range(T):
            gamma = dec[t].clamp(0.0, 1.0)                 # [N]
            A = A * gamma.view(N, 1, 1, 1)
            Bk = Bk * gamma.view(N, 1, 1, 1)
            Vb = Vb * gamma.view(N, 1, 1)

            k_t, q_t = k[t], q[t]                          # [N, K]
            h = bg[t] * k_t                                # [N, K] gated erase key
            u = wg[t] * v[t]                               # [N, V] gated write value

            s_h = self._kron_read_vec(A, Bk, U, Vb, h)     # [N, V]
            r = u - s_h                                    # [N, V]

            c = (k_t * h).sum(-1)                          # [N]
            alpha = self._stable_alpha_vec(c)              # [N]

            s_q = self._kron_read_vec(A, Bk, U, Vb, q_t)   # [N, V]
            kq = (k_t * q_t).sum(-1)                       # [N]
            y = s_q + (alpha * kq).unsqueeze(-1) * r       # [N, V]
            outs.append(y)

            # --- WRITE into slot p (sequential; compaction empties buffer) ---
            new_u = (alpha.unsqueeze(-1) * r)              # [N, V]
            ...
            if p >= P:
                with torch.no_grad():
                    A, Bk, U, Vb, err = self._compact_vec(A, Bk, U, Vb, self.slow_decay)
                comp_err += err
                p = 0

        self._last_comp_err = comp_err
        Y = torch.stack(outs, dim=0)                       # [T, N, V]
    ...
    def _compact_vec(self, A, Bk, U, Vb, slow_decay=0.97):
        ...
        Bm = Q.transpose(-2, -1) @ R_mat             # [N, q, n]
        # top-q SVD of the tiny projected matrix via robust symmetric eigh
        C = Bm @ Bm.transpose(-2, -1)                # [N, q, q] symmetric PSD
        # cuSOLVER crashes on degenerate (near-empty) chains; this q x q matrix
        # is tiny, so do the eigendecomposition on CPU LAPACK which is robust.
        lam, W = torch.linalg.eigh(C.cpu())          # ascending eigenpairs
        lam = lam.flip(-1).clamp(min=0.0).to(C.device)   # -> descending
        W = W.flip(-1).to(C.device)
        Ss = lam.sqrt()                              # singular values [N, q]
        Us = Q @ W                                   # [N, m, q] left singular vectors
        Vs = Bm.transpose(-2, -1) @ (W / Ss.clamp(min=1e-8).unsqueeze(-2))  # [N, n, q]
        kept = (Ss[:, :R] ** 2).sum()
        err = float((total_energy - kept).clamp(min=0.0).item())
    ...
    def forward(
        ...
        **kwargs,  # Accept extra kwargs from Qwen3.5 (cache_params, etc.)
        ...
        b_proj = torch.einsum('hmkd,btd->bthmk', self.W_b,
                              hidden_states)  # [B, T, H, M, K]
        ...
        w_proj = torch.einsum('hmvd,btd->bthmv', self.W_w,
                              hidden_states)  # [B, T, H, M, V]
        ...
        decay_proj = torch.einsum('hmud,btd->bthmu', self.W_decay,
                                  hidden_states)  # [B, T, H, M, 4]
        ...
        decay_factors = decay_per_channel.mean(dim=-1)  # [B, T, H, M]
